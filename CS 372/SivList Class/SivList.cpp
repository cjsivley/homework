...
void List::prev() {
	...
	if (pos < 0) { //boundary condition: underflow
		pos = 0;
		errPrint("prev() Boundary Underflow: Setting pos to 0\n");
	}
	return;
}
...
void List::next() {
	...
	if (pos >= mySize) {
		pos = mySize - 1;
		errPrint("next() Invariant conflict, setting pos to mySize - 1: ");
		cerr << pos << endl;
	}
	return;
}
...
void List::insertBefore(ET x) {
	...
	if (mySize >= CAP) { //if the list is full
		errPrint("insertBefore() list is full, deleting\n");
		return;
	}
...
void List::insertAfter(ET x) {
	...
	if (mySize >= CAP) { //if the list is full
		errPrint("insertAfter() list is full, deleting last entry\n");
		return;
	}
